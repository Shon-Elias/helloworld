...
class HealthBot {
    ...
    */ console.log(color.blue("DATE HERE2"))
    ...
    async DDcurrentClient(dc){
      clientProfile.currentStep[0] = 7
      clientProfile.currentStep[1] = 6.1
      ...
      const message = MessageFactory.attachment(
        CardFactory.heroCard(
          "Great, thanks for that info.",
              ...
              {
                  text: "I see that myEvolv is showing that "+clientProfile.selectedClient+ " does not have any Placement Disruptions active for "+clientProfile.entryDate+".  Do we need to update "+clientProfile.selectedClient+"'s myEvolv record with a new Placement Disruption for "+clientProfile.entryDate+"?"
              })
    );
    await dc.prompt(CONFIRM_PROMPT, message)
    return await dc.endDialog();
    ...
    async DDUpdateDisruptionFunc(dc){
      // clientProfile.currentStep[0] = 7
      clientProfile.currentStep[1] = 6.2
    ...
    async DDDisruptionStartDateFunc(dc){
      clientProfile.currentStep[0] = 7
      clientProfile.currentStep[1] = 6.3
...
// POOOOOOOO
  ...
  async DDSelectedPromptLevelFunc(dc){
    clientProfile.currentStep[0] = 10
    clientProfile.currentStep[1] = 9.1
  ...
  async DDAddingNoteFunc(dc){
    // clientProfile.currentStep[0] = 10
    clientProfile.currentStep[1] = 9.2
  ...
  async DDMoreCarePlansFunc(dc){
    clientProfile.currentStep[0] = 11
  ...
  async DDchooseAdifferentClientFunc(dc){
    clientProfile.currentStep[0] = 12
    ...
    async onTurn(turnContext) {
      ...
      // console.log(color.blue("HERE1"))
        if (turnContext.activity.type === ActivityTypes.Message) {
            ...
            } else if(text === "DD Service Entry" && clientProfile.currentStep[0] === 2) {
                await dc.beginDialog(HELLO_USER)

            // } else if(text === "EBCP STEPS Contact Entry"){

            //   await dc.beginDialog(HELLO_USER)
            }else if(text === "Ok, I'm ready to start" && clientProfile.currentStep[0] === 3){
              // await turnContext.sendActivity({

              //   attachments: CardFactory.adaptiveCard(CalendarCard)
              // })
              await dc.beginDialog("DDmyEvolvServiceEntryDate");
            }else if(!text && clientProfile.currentStep [0] === 4){
              if(entryDate){
          ...
          }else if(clientProfile.clients.includes(text) && clientProfile.currentStep[0] === 5){
            ...
            }else if(text === "Yes, let's update" && clientProfile.currentStep[1] === 6.1){
              clientProfile.askedToUpdate = true
              await dc.beginDialog("DDUpdateDisruption")
            }else if(text === "Not right now" && clientProfile.currentStep[0] === 7){
              await dc.beginDialog("validEndDate")
          }else if(disruption && clientProfile.currentStep[1] === 6.2){
            ...
            }else if(text === "Later" && clientProfile.currentStep[0] === 7){
              await dc.beginDialog("validEndDate")
            }else if(text.includes("You selected:" && clientProfile.currentStep[0] === 8)){
              clientProfile.askedToUpdate = false
              clientProfile.selectedCarePlan = text.split("You selected: ")[1]
              console.log(color.blue("clientProfile.selectedCarePlan"),clientProfile.selectedCarePlan)
              await this.userProfile.set(dc.context, clientProfile)
              await dc.beginDialog("DDSelectedStuffAction")
            }else if(text.includes("Prompt Level" && clientProfile.currentStep[0] === 9)){
              // clientProfile.selectedCarePlan = text.slice("You selected: ")[1]
              // await this.userProfile.set(dc.context, clientProfile)
              await dc.beginDialog("DDSelectedPromptLevel")

            }else if(text === "Yes, I would like to add a note" && clientProfile.currentStep[0] === 10.1){
            ...
            if(startDate && clientProfile.currentStep[1] === 6.2){
